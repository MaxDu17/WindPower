...
with tf.Session() as sess:
    ...
    ckpt = tf.train.get_checkpoint_state(os.path.dirname('../Graphs_and_Results/' + hyp.NAME + '/models/'))
    ...
    log_loss = open("../Graphs_and_Results/" + hyp.NAME + "/GRAPHS/LOSS.csv", "w")  # this is the loss over time
    validation = open("../Graphs_and_Results/" + hyp.NAME + "/GRAPHS/VALIDATION.csv", "w")  # this is the validation
    test = open("../Graphs_and_Results/" + hyp.NAME + "/GRAPHS/TEST.csv", "w")  # this is the test file
    ...
    tf.train.write_graph(sess.graph_def, '../Graphs_and_Results/' + hyp.NAME + '/GRAPHS/',
                         'graph.pbtxt')  # this makes the pbtxt needed for freeze
    writer = tf.summary.FileWriter("../Graphs_and_Results/" + hyp.NAME + "/GRAPHS/",
                                   sess.graph)  # this will write summary tensorboard

    summary = None
    next_state = np.zeros(shape=[2, 1, hyp.cell_dim])  # this initializes the initial "next state"
    ...
    for epoch in range(hyp.EPOCHS):
        ...
        if epoch % 50 == 0 and epoch > hyp.EPOCHS - (
                50 * hyp.FINALJUMP):  # this finds entropic local minima at the end and saves them
            test_local_ = open("../Graphs_and_Results/" + hyp.NAME + "/models/" + str(epoch) + ".csv", 'w')
            test_local = csv.writer(test_local_, lineterminator='\n')

            saver.save(sess, "../Graphs_and_Results/" + hyp.NAME + "/models/V5DUMB", global_step=epoch)

            RMS_loss = 0.0
            next_state_test = np.zeros(shape=[2, 1, hyp.cell_dim])  # initializations
            ...
            for test in range(hyp.Info.TEST_SIZE):  # this will be replaced later
                ...
                next_state_test, output_, loss_ = sess.run([pass_back_state, output, loss],
                                                           ...
                                                           feed_dict={inputs: data, Y: label,
                                                                      init_state: next_state_test})
                RMS_loss += loss_
